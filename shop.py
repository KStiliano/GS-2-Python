...
def finalizar_compra(usuario):
    if not usuario["endereco"].get("estado"):
        ...
        complemento = input(
            "Digite o complemento (deixe em branco se não houver):\n--> ")
    ...
    for id_produto, quantidade in usuario["carrinho"].items():
        ...
        if quantidade <= estoque:
            ...
            itens_comprados.append(
                (produtos[id_produto]["nome"], estoque, preco))
    if usuario["ECs"] >= total_compra:
        usuario["ECs"] -= total_compra
        usuario["saldo_compras"].append(
            {"itens": itens_comprados, "total": total_compra})
        usuario["carrinho"] = {}
    ...
    if not usuario["saldo_compras"]:
        ...
        for id, compra in enumerate(usuario["saldo_compras"], start=1):
            ...
            for item in compra["itens"]:
                print(
                    f"- Produto: {item[0]}, Quantidade: {item[1]}, Preço: {item[2]} ECs /cada")
...
def admin_zone():
    ...
    admin_opcao = forca_opcao(
        "Você deseja adicionar, modificar ou remover produtos? (s/n):\n--> ", ['s', 'n'], "Opção inválida!").lower()
    if admin_opcao == 's':
...
def loja(usuario):
    ...
    while True:
        ...
        if usuario["admin"]:
            admin_zone()
        escolha = input(
            "Digite o ID do produto que deseja comprar, 'compras' para ver suas compras, ou 'sair' para sair:\n--> ").lower()
        if escolha == "sair":
